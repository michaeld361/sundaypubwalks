...
// Create simple HTTP server for Render health checks and manual triggers
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/trigger' || req.url.startsWith('/trigger?')) {
...
async function publishSundayWalkPost() {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      const captionPath = image.localPath.replace('.png', '-caption.txt');
